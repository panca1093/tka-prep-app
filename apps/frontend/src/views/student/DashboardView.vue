...
const avgScore = computed(() => {
  ...
  return results.value.reduce((s, r) => s + r.total_score, 0) / results.value.length
})
  ...
  <div class="dash">
    ...
    <template v-else>
      ...
      <div class="stats-row">
        ...
        <div class="stat">
          <div class="stat-num" :class="avgScore !== null ? scoreClass(avgScore) : ''">
            {{ avgScore?.toFixed(1) ?? '—' }}
          </div>
          <div class="stat-label">Rata-rata Skor</div>
          <div v-if="avgScore !== null" class="stat-bar" :style="`--pct: ${Math.min(avgScore,100)}%`">
            <div class="stat-fill" />
          </div>
        </div>
